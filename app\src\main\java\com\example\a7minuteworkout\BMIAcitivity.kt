...
class BMIAcitivity : AppCompatActivity() { // Class Body Start
    companion object {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        makeVisibleMetricUnitsView() // Calling in onCreate
    ...
    private fun calculateUnits() { // Method to calculate and display BMI
        if(currentVisibleView == METRIC_UNITS_VIEW) {
            if(validateMetricUnits()){ // Validating the metric units
                ...
                displayBMIResult(bmi) // Calling displayBMIResult method to show the bmi
            }
            else{
                Toast.makeText(
                    this ,
            ...
            if(validateUsUnits()) { // Validating the us units
    ...
    private fun makeVisibleMetricUnitsView() { // Method to show metric unit views
    ...
    private fun makeVisibleUsUnitsView() { // Method to show us units views
    ...
    private fun displayBMIResult(bmi: Double){ // Method to display bmi result
        val bmiLabel : String
        val bmiDescription: String

        if(bmi.compareTo(15f) <= 0){ // Checking bmi value and assigning label and description
        ...
        val bmiValue = BigDecimal(bmi).setScale(2 , RoundingMode.HALF_EVEN).toString() // Rounding bmi to 2 decimal places
    ...
    private fun validateMetricUnits() : Boolean { // Method to validate metric unit inputs
        var isValid = true
    ...
    private fun validateUsUnits() : Boolean { // Method to validate us unit inputs
        var isValid = true
